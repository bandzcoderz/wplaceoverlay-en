...
// @version      1.4
// @description  Choose Image, mover, redimensionar, fixar e salvar overlay personalizado no wplace.live com slider de opacidade.
...
function patchUI() {
    ...
    const blendBtn = btn(`Mode: ${overlayMode}`, () => {
        ...
        blendBtn.textContent = `Mode: ${overlayMode}`;
    ...
    const lockBtn = btn(isLocked ? "🔒 Locked" : "🔓 Editable", () => {
        isLocked = !isLocked;
        lockBtn.textContent = isLocked ? "🔒 Locked" : "🔓 Editable";
        if (overlayImg) {
            overlayImg.style.pointerEvents = isLocked ? "none" : "auto";
            overlayImg.style.cursor = isLocked ? "default" : "move";
            overlayImg.style.resize = isLocked ? "none" : "both";
    ...
    const resetBtn = btn("Redefine Overlay", () => {
        localStorage.clear();
    ...
    const centerBtn = btn("Centralize on screen", () => {
        if (overlayImg) {
    ...
    const restoreSizeBtn = btn("Original Size (reset)", () => {
        if (overlayImg) {
